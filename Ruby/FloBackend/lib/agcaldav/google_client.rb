module AgCalDAV
  class GoogleClient < Api::Web::BaseController
    ...
    def getlistcalendars
      # reqUrl =  @url[0..-6] #substring /user in @url
        res = nil
        __create_http.start {|http|
        ...
        if res.code.to_i().between?(200, 299)
          mparser.parse_calendar(res, 1, @url) #googleCalendarsParser(res)
        else
          return [{ :error => { :code => res.code, :message => res.message}}]
        end
    end
    
    def  get_g_calendar(cal_uri)
      res = nil
      __create_http.start {|http|
      ...
      if res.code.to_i().between?(200, 299)
        ...
        xml.elements.each('D:multistatus/D:response') do |elm|
          ...
          if e['D:propstat/D:prop/D:displayname'] && e['D:href'] then
            write_content = e['D:propstat/D:prop/D:current-user-privilege-set/D:privilege/D:write-content']
            ...
            item = { 'name' => e['D:propstat/D:prop/D:displayname'].text,
                    'href' => e['D:href'].text,
                    'color' => e['D:propstat/D:prop/ical:calendar-color'].text,
    ...
    def report(calendar, component_type, range)
        res = nil
        __create_http.start {|http|
            req = Net::HTTP::Report.new(calendar, initheader = {'Content-Type'=>'application/xml; charset=UTF-8'} )
            if @authtype == 'basic'
              req.basic_auth @user, @password
            elsif @authtype == 'bearer'
              req['Authorization'] = "Bearer #{@token}"
            ...
            if component_type == 'VEVENT'
              req.body = AgCalDAV::Request::ReportVEVENT.new( 'time-range', nil, range ).to_xml
            elsif component_type == 'VTODO'
              req.body = AgCalDAV::Request::ReportVTODO.new( 'time-range', nil, range ).to_xml
            elsif component_type == 'VJOURNAL'
              req.body = AgCalDAV::Request::ReportVJOURNAL.new( 'time-range', nil, range ).to_xml
            end
       ...
       if res.code.to_i().between?(200, 299)
          mparser.parse_calendar_object(res, 1, calendar)
       else
          return [{ :error => { :code => res.code, :message => res.message}}]
       end
    end
    
    #Create new and update calendar object
