...
class Api::Web::GoogleCaldavController < Api::Web::BaseController
  ...
  def getlist_calendars
    ...
    results = Array.new
    results = @gcal.getlistcalendars if @gcal.present?
    errors = Array.new
    if results
      if results.length
  ...
  def get_calendar_objects_by_calendar_uri
    ...
    errors = Array.new
    results = @gcal.report(cal_uri, objType, minRange..maxRange)
    if results
      if results.length
        if !results[0].nil? && !results[0][:error].nil?
          # && results[0][:error][:code].to_i == 401
    ...
    if !errors.empty?
      ...
      res_objs = Api::Web::Utils.convert_calendar_obj(results, true, nil)[:data]
      res[:data] = res_objs
    end
...
#   
  ...
  def update_calobj_by_ical_str
    ...
    results = @gcal.update_calobj_by_ical_str(uuid, calobj, caluri)   #call this method from client.rb
