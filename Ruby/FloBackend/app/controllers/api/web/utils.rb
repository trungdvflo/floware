...
class Api::Web::Utils
  ...
  #generate signature
  def self.generate_signature(email = '', digesta1_pass = '', appreg = '')
  ...
  # def self.create_task_default(folder_id = 0, task_title = "", task_location = "", uuid = '', task_des = '', task_due = '', task_subs = nil)
    ...
    # task[:description] = (task_des.to_s != '') ? task_des : ''
  ...
  # def self.create_note_default(folder_id = 0, note_title = "", uuid = '', description = '')
    ...
    # # uuid = UUID.new.generate
    # note[:uuid] = uuid
    # note[:summary] = (note_title.to_s != '') ? note_title.to_s : "Note default"
    # note[:dtstart] = Time.now.to_datetime.to_s
    # note[:folderid] = folder_id.to_s
    # note[:description] = (description.to_s != '') ? Nokogiri::HTML(description).text : "This is Note description"
    # notecnt = (description.to_s != '') ? description.to_s : "This is note content"
    # note[:x_lcl_notecontent] = Base64.encode64(notecnt)
    # return note
  # end
  ...
  # ]'
    ...
    # setting[:alert_before] =  0 #Time of Start #600 #60 mins before
    ...
    # setting[:default_todo_alert] = 0 #date of due option
  ...
  def self.convert_calendar_obj(records, isHashObj = false, trashed_items = nil, opts = nil)
    ...
    if records.present?
      records.each do |rc|
        ...
        calObjs.each do |o|
          if opts.present? && opts[:prev].present? && opts[:next].present?
          ...
          if type == API_VTODO || type == API_VEVENT
            ...
            if type == API_VEVENT || o.x_lcl_stask
              ...
              elsif type == API_VTODO # for case o.x_lcl_stask
                customObj['order_number'] = record['order_number'] || ''
              end
            ...
            if isHashObj
              ...
              if type != API_VJOURNAL && !customObj['tzid'].nil? && o.due.present?
              ...
              if type != API_VJOURNAL && !customObj['tzid'].nil? && o.completed.present?
            ...
            if type == API_VEVENT || o.x_lcl_stask
              if opts.present? && opts[:prev].present? &&
                  opts[:next].present? &&
                  !customObj['rrule'] &&
                  !is_multi_day_event(customObj)
