class Api::SubscriptionController < Api::BaseController
  ...
  def create
    ...
    if subs
      ...
      if subs[:transaction_id]
        ...
        spObj = SubPurchase.where(transaction_id: sp.transaction_id)
        if spObj and spObj.length  > 0
          hasError = 1
          info[:error] = API_TRANSACTION_EXISTED
          info[:message] = API_MSG_TRANSACTION_EXISTED
        end
      else
        hasError = 1
        info[:error] = API_TRANSACTION_INVALID
        info[:message] = API_MSG_TRANSACTION_INVALID
      end
        ...
        if spReceipt and spReceipt.length > 0
          ...
          if stt.to_i == 0 #verify OK
            if sp.save
              ...
              #send mail thank you user upgrade account
              user = User.find user_id
              if user
                subs = Subscription.find(sp.subID)
                upgradeTo = subs ? subs.name : ""
                mails = [{ :to_email => user.email, 
                           :subject => API_SUBS_THANKS_UPGRADE_SUBJ, 
                           :template => API_SUBS_THANKS_UPGRADE_MAIL,
                           :upgradeTo => upgradeTo }]
                #daemon will send this email  
                SendMail.insert_send_mail(mails)
                
                #add to push notification thank you
                body = MSG_THANK_YOU_UPGRADE
                body = body.gsub("@upgradeTo@", upgradeTo)
                msg = {
                  :body => body,
                  :floNotificationType => 1
                }
                notiItem = {
                  :user_id => user.id,
                  :email => user.email,
                ...
                PushNotiqueue.insert_push_noti(notiItem)
                
                #reset the number sent mail notice for user
                ...
                #reset disable account
                if user.disabled.to_i == 1
                  user.disabled = 0
                  user.save
                end
                
                #check logic: if upgrade subscription higher, it will apply now
                SubPurchase.check_to_apply_subs_higher(user.id, sp)
