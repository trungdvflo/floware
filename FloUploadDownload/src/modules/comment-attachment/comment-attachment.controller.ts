import {
  ...
  , Res
  , UploadedFile
  , UseFilters, UseInterceptors, UsePipes } from '@nestjs/common';
import { FileInterceptor } from '@nestjs/platform-express';
import { ApiBody, ApiConsumes, ApiOperation, ApiTags } from '@nestjs/swagger';
import * as AWS from 'aws-sdk';
import { Response } from 'express';
import { MSG_ERR_DOWNLOAD } from '../../common/constants/message.constant';
...
import { BaseBadRequestValidationFilter, UnknownExceptionFilter } from '../../common/filters/validation-exception.filters';
...
export class CommentAttachmentController {
    ...
    try {
      if (!file) {
        throw new BadRequestException(
          buildFailItemResponse(ErrorCode.BAD_REQUEST, MSG_ERR_DOWNLOAD.PLS_SELECT_FILE));
      }
      ...
      const data = await this.service.fileSingleUpload(validateDTO, file, ext, req.user);
      res.send({ data });
    } catch (err) {
      throw err;
    }
  ...
  async delete(
    ...
    const result = await this.service.deleteFile(data, req.user);
...
export const multerOptions = {
  ...
  limits: {
      fileSize: process.env.LIMIT_BODY_SIZE? (+process.env.LIMIT_BODY_SIZE)*1024*1024
      : 20*1024*1024,
  },
};