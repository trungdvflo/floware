CREATE PROCEDURE `c2023_getListCommentV3`(pnCollectionId  BIGINT(20)
...
BEGIN
  ...
  DECLARE nFilterType   TINYINT(1) DEFAULT ifnull(pnFilterType, 0);
...
LEFT JOIN mention_user mu ON (cm1.mention_user_id = mu.id)
     JOIN (
          ...
          SELECT co.id collection_id, co.calendar_uri owner_calendar_uri, co.user_id owner_user_id
           ...
           WHERE co.id = IF(ifnull(pnCollectionId, 0) > 0, pnCollectionId, co.id)
             AND co.`type` = IF(ifnull(pnCollectionId, 0) > 0 OR ifnull(pvObjectUid, '') <> '', co.`type`, 3) -- owner GET BY uid? dont need TO share
   ...
   WHERE ca.collection_id = IF(ifnull(pnCollectionId, 0) > 0, pnCollectionId, ca.collection_id)
     ...
     AND cm.is_draft = IF(cm.user_id = pnUserId, cm.is_draft, 0)
