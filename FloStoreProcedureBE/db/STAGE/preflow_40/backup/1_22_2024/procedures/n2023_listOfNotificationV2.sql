  ...
  END IF;
   SELECT cn.id, cn.email, cn.collection_id, cn.object_uid, cn.object_type
        -- VIRTUAL action: 63 - MENTIONED
         ,IF((cn.action = 6 OR cn.action = 61) AND un.has_mention = 1 AND ifnull(mu.id, 0) > 0, 63, cn.action) `action`
...
LEFT JOIN url ON (cn.object_uid = url.uid AND cn.object_type = 'URL')
    ...
    WHERE cn.collection_id = IF(ifnull(pnCollectionId, 0) > 0, pnCollectionId, cn.collection_id)
      ...
      AND CASE WHEN cn.action = 61 -- only SHOW notification edited FOR mentioned member
               THEN ifnull(mu.id, 0) > 0 && un.has_mention > 0
               ELSE 1
           END
      ...
      AND (find_in_set(0, vAssignment)
           ...
           -- 1: NOT Assigned 
           OR (find_in_set(1, vAssignment) AND (ifnull(cn.assignees, '') = '') OR cn.action = 18)
           -- 2: Assigned TO Me 
