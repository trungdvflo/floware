CREATE PROCEDURE `c2023_createChannel`(pnUserId         BIGINT(20)
...
BEGIN
  ...
  -- CHECK existed creator
    ...
    (user_id, title, uid, room_url, created_date, updated_date)
    VALUES
    (pnUserId, pvTitle, pvUid, ifnull(pvRoomUrl, ''), pnCreatedDate, pnUpdatedDate);
    #
    ...
    INSERT INTO conference_member
      (user_id, channel_id, email, is_creator, title, description, avatar, vip, created_date,    updated_date)
    VALUES
      (pnUserId, nchannelId, pvEmail,          1, pvTitle, pvDescription, pvAvatar, ifnull(pnVip, 0), pnCreatedDate, pnUpdatedDate);
