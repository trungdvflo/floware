...
BEGIN
  ...
  DECLARE vApiName     VARCHAR(255) DEFAULT CASE pvApiName WHEN 'file' THEN 'file_member' ELSE vApiName END;
  ...
  api_loop: LOOP
    ...
    END IF;
    # 2. main UPDATE
    SET nReturn = m2023_insertAPILastModify(pvApiName, nUserId, pnLastModifyDate);
    --
    -- IF we send api name comment OR something ELSE ...
    -- ... we also need TO send FOR the collection activity too
    IF pvApiName = 'collection_comment'
       OR pvApiName = 'collection_history'
       THEN 
      # main UPDATE
