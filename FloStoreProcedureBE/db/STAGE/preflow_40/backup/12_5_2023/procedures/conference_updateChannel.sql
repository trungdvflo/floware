CREATE PROCEDURE `conference_updateChannel`(memberId      BIGINT(20)
                                                              ,userId        BIGINT(20)
...
--
  ...
  ELSE
    ...
    --
    IF ifnull(isCreator, 0) > 0 AND NOT isnull(roomUrl) THEN
      --
     UPDATE conference_channel cc
        SET cc.room_url = ifnull(roomUrl, '')
      WHERE cc.id = nChannelId;
      --
    END IF;
    --
    SELECT cm.id id, cc.room_url
             ,cm.vip, cm.email, cm.is_creator, cm.uid, cm.created_date, cm.updated_date, ifnull(cm.revoke_time,0) revoke_time
