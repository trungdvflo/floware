...
--
  ...
  SELECT ifnull(max(cm1.id), -1) id, cm1.channel_id, chx.room_url
    ...
    JOIN (
          SELECT cc.id, cc.room_url, cc.uid, cc.title, ch.updated_date
            FROM conference_channel cc
            JOIN conference_member cm ON (cc.id = cm.channel_id)
       LEFT JOIN (
                  SELECT ch1.member_id, MAX(ch1.updated_date) AS updated_date
          ...
          HAVING (
           ...
           ORDER BY GREATEST(ifnull(ch.updated_date, 0), cc.updated_date) DESC 
           LIMIT 1
