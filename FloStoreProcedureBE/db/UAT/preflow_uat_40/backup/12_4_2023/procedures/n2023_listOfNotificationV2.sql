  ...
  END IF;
   SELECT cn.id, cn.email, cn.collection_id, cn.object_uid, cn.object_type
        -- VIRTUAL action: 63 - MENTIONED
         ...
         ,cn.content, cn.comment_id, cn.created_date
         ,greatest(cn.action_time, ifnull(un.action_time, 0)) action_time
         ...
         ,(CASE
          ...
          ,ifnull(un.`status`, 0) read_status
          ,cn.assignees
...
LEFT JOIN url ON (cn.object_uid = url.uid AND cn.object_type = 'URL')
    ...
    WHERE cn.collection_id = IF(ifnull(pnCollectionId, 0) > 0, pnCollectionId, cn.collection_id)
      ...
      AND (find_in_set(0, vAssignment)
           ...
           -- 1: NOT Assigned 
           OR (find_in_set(1, vAssignment) AND ifnull(cn.assignees, '') = '')
           -- 2: Assigned TO Me 
           OR (find_in_set(2, vAssignment) AND find_in_set(pvUsername, ifnull(cn.assignees, 'NA')))
           -- 3: Assigned BY Me
