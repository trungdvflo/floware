...
BEGIN
  ...
  IF bAlmComment THEN
     --
     SET nReturn = modify_insertAPILastModify('collection_comment', pnUserId, dDeletedDate);
     --
  ELSEIF ifnull(pvItemType, 'NA') <> 'NA' THEN
    --
    SET nReturn = modify_insertAPILastModify(lower(pvItemType), pnUserId, dDeletedDate);
    --
  END IF;
  ...
  IF ifnull(pnCollectionId, 0) > 0 THEN
    --
    SET nReturn = collection_generateDeletedItemForMember(pvItemType, pnCollectionId, pnItemId, dDeletedDate);
    --
    IF bAlmComment THEN
      --
      SET nReturn = collection_sendLastModifyShare('collection_comment', pnCollectionId, dDeletedDate);
      --
      ELSEIF ifnull(pvItemType, 'NA') <> 'NA' THEN
      --
      SET nReturn = collection_sendLastModifyShare(lower(pvItemType), pnCollectionId, dDeletedDate);
      --
    END IF;
    --
  END IF;
