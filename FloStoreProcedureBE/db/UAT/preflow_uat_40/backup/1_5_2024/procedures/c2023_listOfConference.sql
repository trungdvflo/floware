...
BEGIN
  ...
  SELECT cm.id, cm.channel_id, cc.uid, cm.email, cm.is_creator, cm.description
        ,cm.vip, cm.revoke_time, cm.updated_date, cm.created_date, cm.join_time
    ...
    WHERE cm.user_id = pnUserId
      ...
      ORDER BY
        (CASE
           ...
           WHEN isnull(pnMinId) AND isnull(pnModifiedGTE) AND INSTR(vSort, "-") THEN -- DEFAULT WHEN no ASC active
             --
             CASE vFieldSort
               --
