...
BEGIN
  --
  DECLARE vDomain VARCHAR(20) DEFAULT '@flouat.net';
  -- owner
  (SELECT cn.id, co.user_id user_id
    ,st.notification_clean_date, cn.created_date, unix_timestamp(now(3)) - st.notification_clean_date vtime
...
LEFT JOIN user_notification un ON (cn.id = un.collection_notification_id AND un.user_id = co.user_id)
    WHERE co.id = ifnull(pnCollectionID, co.id) -- 1. immediately DELETE via worker collection
      AND CASE WHEN pnCollectionID IS NULL 
               THEN cn.created_date < unix_timestamp(now(3)) - st.notification_clean_date -- 2. clean WHEN reach setting time LIMIT
           AND uu.email IN (
