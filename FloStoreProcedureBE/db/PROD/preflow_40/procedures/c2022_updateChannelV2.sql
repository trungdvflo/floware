CREATE PROCEDURE `c2022_updateChannelV2`(memberId      BIGINT(20)
                                                              ,userId        BIGINT(20)
  ...
  ELSE
    ...
    IF ifnull(isCreator, 0) > 0  THEN
     ...
     UPDATE conference_channel cc
        SET cc.room_url = CASE WHEN isnull(roomUrl) THEN cc.room_url ELSE roomUrl END
           ,cc.enable_chat_history = CASE WHEN isnull(pnEnableChatHistory) THEN cc.enable_chat_history ELSE pnEnableChatHistory END
