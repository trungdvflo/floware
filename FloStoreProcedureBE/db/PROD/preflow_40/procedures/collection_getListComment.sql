CREATE PROCEDURE `collection_getListComment`(nCollectionId    BIGINT(20)
...
BEGIN
  ...
  SELECT cm.id AS id, cm.collection_activity_id, cm.email, cm.action, cm.action_time, cm.comment
    ...
    LEFT JOIN (SELECT lfc.source_id, lfc.user_id
                ,CONCAT('[',GROUP_CONCAT(
                  JSON_OBJECT(
                    'id', fc.id
                    ,'uid', fc.uid
            ...
            FROM linked_file_common lfc 
            JOIN file_common fc ON (lfc.file_common_id = fc.id)
           WHERE lfc.source_type = 'COMMENT'
           GROUP BY source_id) fi ON (fi.source_id = cm.id AND fi.user_id AND cm.user_id)
    JOIN (
