...
BEGIN
  ...
  SELECT cm.id, cc.uid, cm.channel_id, cm.user_id, cm.email
    INTO nMemberID, vUid, nchannelId, nUserId, vEmail
  ...
  IF ifnull(nMemberID, 0) > 0 THEN
    ...
    END IF;
    --
    SELECT nMemberId id, nCount, nchannelId channel_id, nUserId user_id, vEmail email;
    --
  END IF;
  --
  SELECT 0 id;
