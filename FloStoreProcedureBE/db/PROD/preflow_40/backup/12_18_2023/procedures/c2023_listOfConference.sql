...
BEGIN
  ...
  --
  SELECT cm.id, cm.channel_id, cm.uid, cm.email, cm.is_creator, cm.description
        ,cm.vip, cm.revoke_time, cm.updated_date, cm.created_date
        ,cc.room_url, COALESCE(cm.title, cc.title, '') AS title
...
LEFT JOIN linked_collection_object lco ON (cm.uid = lco.object_uid AND lco.object_type = 'CONFERENCING' 
