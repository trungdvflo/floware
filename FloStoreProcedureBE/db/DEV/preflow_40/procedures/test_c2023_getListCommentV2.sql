CREATE PROCEDURE `test_c2023_getListCommentV2`(pnCollectionId  BIGINT(20)
...
LEFT JOIN mention_user mu ON (cm1.mention_user_id = mu.id)
   ...
   WHERE ca.collection_id   = IF(ifnull(pnCollectionId, 0) > 0, pnCollectionId, ca.collection_id)
     ...
     -- QUANG BE
     AND cm.created_date    < IF(IFNULL(pnBeforeGTE, 0) > 0, pnBeforeGTE, unix_timestamp() + 1)
