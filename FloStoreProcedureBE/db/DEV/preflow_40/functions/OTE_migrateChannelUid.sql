CREATE FUNCTION `OTE_migrateChannelUid`() RETURNS INT(11)
BEGIN
    ...
    SELECT cc.id, cm.uid, cm.user_id
      FROM conference_channel cc
      JOIN conference_member cm ON (cc.id = cm.channel_id AND cc.user_id = cm.user_id)
      WHERE cc.uid = ''
    --  AND cm.is_creator = 1
      ;
    # END of: main script
