...
const ReplicationConfig = (options, pool) => {
  ...
  if (_.isEmpty(replicationReadOptions) === true) {
    replicationReadOptions = [{ ...replicationWriteOptions }];
  }
  ...
  try {
    return new Promise((resolve, reject) => {
      _.forEach(connections, (v, k) => {
        try {
          if (v.engine === 'sequelize') {
            ...
            if (options.min) pool.min = options.min;
            if (options.acquire) pool.acquire = options.acquire;
            if (options.idle) pool.idle = options.idle;
      ...
      Object.keys(db).forEach((modelName) => {
        ...
        const model = _.find(store.models, { modelName });
