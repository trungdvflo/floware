...
import { nameSwagger } from '../../common/swaggers/file-member.swagger';
import {
  DataRespond,
...
import { DeleteFileDTO, DeleteFileSwagger } from './dtos/file-member-delete.dto';
...
@Controller(routestCtr.fileMemberCtr.mainPath)
@ApiTags(nameSwagger)
export class FileMemberController {
  constructor(private readonly fileMemberService: FileMemberService) {}
  ...
  async index(
    ...
    const dataRespond = await this.fileMemberService.getAllFiles(filter, req.user.userId);
    return dataRespond;
  }

  @Get(routestCtr.fileMemberCtr.downloadPath)
  @UsePipes(new BatchValidationPipe())
  async download(@Query() reqParam: FileMemberDownloadDTO, @Res() res: Response, @Req() req) {
    const { stream, resContent } = await this.fileMemberService.downloadSingleFile(
      reqParam,
      req.user.userId
    );
    res.set(resContent);
  ...
  @Post(routestCtr.fileMemberCtr.uploadPath)
  @ApiConsumes('multipart/form-data')
  @UseInterceptors(FileInterceptor('file', multerOptions))
  @ApiBody({ type: FileMemberDTO })
  async uploadedFile(
    @Body(new FileValidationPipe()) postData: FileMemberDTO,
    @UploadedFile()
    ...
    const validateDTO: FileMemberDTO = Object.assign(new FileMemberDTO(), postData);
    ...
    const data = await this.fileMemberService
  ...
  @Post(routestCtr.fileMemberCtr.deletePath)
  async delete(
    ...
    const result = await this.fileMemberService.deleteMemberFile(data, req as IReq);
