...
export class S3Utility {
   ...
   * @param {*} source
   */
  FileExist(source: string, Bucket: string = this.Bucket) : Promise<boolean> {
   ...
   */
  DownloadUrl(source: string, Expires: number = 10, Bucket: string = this.Bucket)
    : Promise<ObjectS3> {
    return new Promise((resolve, reject) => {
      try {
   ...
   * @param {*} source
   */
  DownloadFile(source: string, Bucket: string = this.Bucket) {
    return new Promise((resolve, reject) => {
      try {
        this.s3.getObject({
          Bucket,
