...
module.exports.Start = (AwsConfig) => {
  return new Promise((resolve, reject) => {
    try {
      ...
      const accountId = _.get(AwsConfig, 'ACCOUNT_ID', process.env.AWS_ACCOUNT_ID);
      const region = _.get(AwsConfig, 'REGION', process.env.AWS_REGION);
      const QueueName = _.get(AwsConfig, 'QUEUE_NAME', process.env.AWS_SQS_QUEUE_NAME);

      if (_.isEmpty(QueueName) === true) {
        resolve(false);
      }
      const sqsConfig = {
        accountId,
