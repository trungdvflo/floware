...
if [ -f /var/www/html/.devops/conf/nginx/nginx.conf ]; then
  cp /var/www/html/.devops/conf/nginx/nginx.conf /etc/nginx/nginx.conf
fi

if [ -f /var/www/html/.devops/conf/nginx/nginx-site.conf ]; then
  cp /var/www/html/.devops/conf/nginx/nginx-site.conf /etc/nginx/sites-available/default.conf
fi

if [ -f /var/www/html/.devops/conf/nginx/nginx-site-ssl.conf ]; then
  cp /var/www/html/.devops/conf/nginx/nginx-site-ssl.conf /etc/nginx/sites-available/default-ssl.conf
fi
...
if [[ "$RUN_SCRIPTS" == "1" ]] ; then
  if [ -d "/var/www/html/.devops/scripts/" ]; then
    # make scripts executable incase they aren't
    chmod -Rf 750 /var/www/html/.devops/scripts/*; sync;
    # run scripts in number order
    for i in `ls /var/www/html/.devops/scripts/`; do /var/www/html/.devops/scripts/$i ; done
  else
