...
use Sabre\DAV\Auth\Backend\AbstractDigest as SabreAbstractDigest;

abstract class AbstractDigest extends SabreAbstractDigest {
     ...
     * @var string
     */
    protected $realm = 'LeftCoastLogic';
    ...
    function check(RequestInterface $request, ResponseInterface $response) {
        ...
        // fix bug auth sabre
        try{
            $username = $digest->getUsername() ?? null;
        }catch(\Throwable $e){
            $username = null;
        }
        ...
        // No userInfo was given
        if (!$userInfo || is_null($userInfo)) {
            return [false,"User information does not exist"];
        }
        // Check status disabled
        if ($userInfo['disabled'] == 1) {
            return [false,"User account is disabled and waiting for delete"];
        }
        $hash = $userInfo['digesta1'];
