const config = {
    tableName: 'users'

};
const Model = {
    datastore: 'flowdataSequelize',
    tableName: config.tableName,
    attributes: (sequelize, DataTypes) => {
        const Users = sequelize.define(config.tableName, {
            id: {
                type: DataTypes.INTEGER,
                primaryKey: true,
                autoIncrement: true
            },
            username: {
                type: DataTypes.STRING,
                required: true
            },
            email: {
                type: DataTypes.STRING,
                required: true
            },
            digesta1: DataTypes.STRING,
            secondary_email: {
                type: DataTypes.STRING,
                required: true
            },

            password: {
                type: DataTypes.STRING,
                required: true
            },
            fullname: {
                type: DataTypes.STRING,
                required: true
            },
            description: {
                type: DataTypes.STRING
            },
            rsa: {
                type: DataTypes.STRING,
                required: true
            },
            appreg_id: {
                type: DataTypes.STRING,
                required: true
            },

            domain_id: {
                type: DataTypes.INTEGER,
                default: 0,
                required: true
            },
            disabled: DataTypes.INTEGER,
            created_date: DataTypes.DOUBLE(13, 3),
            updated_date: DataTypes.DOUBLE(13, 3)
        }, config);

        Users.associate = (models) => {
            Users.hasMany(models.groups_users, {
                as: 'groupUsers',
                foreignKey: 'user_id'
            });

            Users.hasMany(models.releases, {
                as: 'releases',
                foreignKey: 'user_id'
            });

            Users.hasMany(models.rules, {
                as: 'rule',
                foreignKey: 'user_id'
            });

            Users.hasMany(models.projects, {
                as: 'project',
                foreignKey: 'user_id'
            });

            Users.hasMany(models.rules_filter_actions, {
                as: 'rules_filter_actions',
                foreignKey: 'user_id'
            });

            Users.hasMany(models.rules_filter_conditions, {
                as: 'rules_filter_conditions',
                foreignKey: 'user_id'
            });
        };
        return Users;
    }
};

module.exports = Model;
