...
const { DOWNLOAD_PATH } = require('../../constants/AppsConstant');

class GoodHapiGraylog extends Stream.Writable {
  constructor({
    host, port, facility, adapter = 'udp'
  }) {
    ...
    this.log = log.setConfig({
      ...
      transform: [
        (rawLog) => { // unwind an error   
          ...
          const {
            tags, event, statusCode, errorMessage
          } = message;

          const path = _.get(message, 'path', '');
          const method = _.get(message, 'method', '');
          if (path.indexOf(DOWNLOAD_PATH) >= 0) {
            return DOWNLOAD_PATH;
          }

          let result = _.clone(rawLog);
          switch (event) {
            case 'error': {
              result = LogsUtil.Filter(message, `[Exception]${statusCode ? `[${statusCode}]` : ''} ${message.error.name}`);
              break;
            }
            case 'response': {
              result = LogsUtil.Filter(message, `${statusCode ? `[${statusCode}]` : ''}${method ? `[${message.method.toUpperCase()}]` : ''} Response for ${process.env.BASE_URL}${path}`);
              break;
            }
            default: {
              if (_.isEmpty(tags) === false && tags.indexOf('SystemErrorTracking') >= 0) {
                result = LogsUtil.Filter(message, `[Exception]${statusCode ? `[${statusCode}]` : ''} ${process.env.BASE_URL}${path} : ${errorMessage || ''}`);
                break;
              }
              result = LogsUtil.Filter(message, `Log event for tags: ${CircularJSON.stringify(message.tags)}`);
              break;
            }
          }
